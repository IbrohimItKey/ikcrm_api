...
class ForTheBuilderController extends Controller
{
    ...
    public function index()
    {
          ...
          // Deal
        $new_users=DB::table('forthebuilder.deals as dt1')
        ->join('forthebuilder.clients as dt2', 'dt2.id', '=', 'dt1.client_id')
        ->where('dt1.type',Constants::FIRST_CONTACT)
        // ->distinct('dt1.client_id')
        ->count();
        // dd($new_user);
        $in_negotiations=DB::table('forthebuilder.deals as dt1')
        ->where('dt1.type',Constants::NEGOTIATION)
        // ->distinct('dt1.client_id')
        ->count();
        $make_deal=DB::table('forthebuilder.deals as dt1')
        ...
        ->count();
         ...
         $today=DB::table('forthebuilder.task as task')
        //  ->join('forthebuilder.clients as dt2', 'dt2.id', '=', 'dt1.client_id')
        ...
        //  dd($datetime);
         $tomorrow=DB::table('forthebuilder.task as task')
        //  ->join('forthebuilder.clients as dt2', 'dt2.id', '=', 'dt1.client_id')
        ...
        // dd($day_after_a_week);
        $week=DB::table('forthebuilder.task as task')
        // ->join('forthebuilder.clients as dt2', 'dt2.id', '=', 'dt1.client_id')
        ...
        $full_task=DB::table('forthebuilder.task as task')
        // ->join('forthebuilder.clients as dt2', 'dt2.id', '=', 'dt1.client_id')
        ->where('task.task_date','>=',$date)
        ->where('task.status',0)
        ->count();
        ...
        $overdue_tasks=DB::table('forthebuilder.task as task')
        // ->join('forthebuilder.clients as dt2', 'dt2.id', '=', 'dt1.client_id')
        ->where('task.task_date','<',$date)
        ->where('task.status',0)
        ->count();
        ...
        $house_count=DB::table('forthebuilder.house as house')
        ->where('house.deleted_at',null)
        ->count();

        $house_flat_status_free=DB::table('forthebuilder.house_flat as house_flat')
        ->where('house_flat.deleted_at',null)
        ->where('house_flat.status',Constants::STATUS_FREE)
        ->count();

        $house_flat_status_booking=DB::table('forthebuilder.house_flat as house_flat')
        ->where('house_flat.deleted_at',null)
        ->where('house_flat.status',Constants::STATUS_BOOKING)
        ->count();

        $house_flat_status_sold=DB::table('forthebuilder.house_flat as house_flat')
        ->where('house_flat.deleted_at',null)
        ->where('house_flat.status',Constants::STATUS_SOLD)
        ->count();
        // dd($house_flat_status_sold);

        // $installment_count=DB::table('forthebuilder.installment_plan as installment_plan')
        // ->count();
        ...
        $month_prices = DB::table('forthebuilder.deals as dt1')
            ->join('forthebuilder.house_flat as dt2', 'dt2.id', '=', 'dt1.house_flat_id')
            ->join('newhouse.users as dt3', 'dt3.id', '=', 'dt1.user_id')
